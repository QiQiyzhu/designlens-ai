...
**Reviewed research intake:** [Local cleanup → inspect retained evidence → approve or revoke cloud use](docs/research-intake-upgrade.md). Import previews never call a model or save raw research. Real imports require a review bound to the current input; cloud use also requires a per-source approval bound to the retained content hash. Local pattern suggestions have false positives and blind spots, so manual review remains essential. [Frozen 12-case × 3-variant development protocol](docs/research-ablation-protocol.md) separates privacy exposure from task coverage; its paid run is pending.
...
After building and starting FastAPI, open `http://127.0.0.1:8001/` for the complete same-origin demo. Current validation: **76 backend tests and 7 browser flows passed**, including review invalidation, retained-content privacy, per-source approval/revocation and bounded evaluation execution. [JUnit](reports/research-intake-tests.xml) · [browser evidence](reports/research-intake-browser.json). The [43-test original baseline](docs/validation-report.md), [63-test transport extension](reports/remote-provider-tests.xml) and 48 deterministic evaluation executions remain historical records. From frontend, `npm test` runs browser checks against an isolated database; `node scripts/smoke.mjs` checks the built local app.
