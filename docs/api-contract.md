...
|---|---|
| Local preview | `POST /api/sources/preview` with the import payload; returns `{preview_digest,sources,changes,record_count,policy_version,network_calls:0,persisted:false,limitations}`; no model or persistence |
| Import | `POST /api/sources/import` `{filename,content,type,participant,segment,is_demo:true,consent_confirmed:false,redaction_terms:[],privacy_review_digest?}`; real sources require consent and the unchanged preview digest; returns `{sources,imported}` |
| Retained source | `GET /api/sources/{id}` returns immutable cleaned content and reviewed provenance metadata |
| Cloud review | `POST /api/sources/{id}/remote-review` `{approved,content_sha256,note,consent_confirmed}`; note 10–2000 chars; approval requires consent and reviewed intake; stale hash HTTP 409; update and audit event commit together |
...
| Decision export | `GET /api/export`; includes retained source text, still consent-sensitive |

Source fields: `id/source_id,type,participant,timestamp,segment,content,metadata,is_demo,privacy_review,remote_review`. Privacy review binds the input digest, cleanup policy and retained content hash; remote review binds approval, purpose note and timestamp to that retained content hash. `redaction_terms` accepts at most 30 literal terms, each 2–200 characters, and is never persisted. Server-generated review state cannot be supplied through source metadata. Legacy sources without privacy review need reimport before remote approval. Insight fields: `id,title,observation,pain_point,need,insight,evidence_ids,evidence:[{evidence_id,quote}],confidence,status,review_note,is_demo,generated_by,created_at`. Opportunities include score assumptions plus `ai_recommendation`, `recommendation_basis`, `human_decision:{decision,reason}`.
