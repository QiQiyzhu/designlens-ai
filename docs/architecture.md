...
## Provider boundary
No key is needed in extractive mode. Remote mode requires a provider URL, model and key. Source text is framed as data, output is parsed, exact source/quote checks run, and remote failures are recorded without silently substituting a successful demo result. Token usage is null if unavailable; cost is unknown without a billing basis. Real evidence stays local unless the owner explicitly sets the remote-data permission environment variable.
